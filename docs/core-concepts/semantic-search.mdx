...
description: "Find feedback using natural language queries. Search by meaning, not just keywords—\"checkout problems\" will find \"payment failed\", \"cart issues\", and \"can't complete order\"."
---
...
### 1. Enable Embeddings[​](#1-enable-embeddings "Direct link to 1. Enable Embeddings")

Add the embedding model to your environment:

```
# Required - same OpenAI key as enrichmentSERVICE_OPENAI_API_KEY=sk-your-api-key-here# Optional - embedding model (default: text-embedding-3-small)SERVICE_OPENAI_EMBEDDING_MODEL=text-embedding-3-small
```
...
### 2. Search Your Feedback[​](#2-search-your-feedback "Direct link to 2. Search Your Feedback")

Use the semantic search API:

```
curl "http://localhost:8080/v1/feedback-records/search?query=checkout+problems&limit=10" \  -H "X-API-Key: your-api-key"
```

**Response:**

```
{  "results": [    {      "id": "01abc...",      "value_text": "Payment failed during checkout. Very frustrating.",      "field_label": "What went wrong?",      "sentiment": "negative",      "emotion": "frustration",      "topics": ["checkout", "payment"],      "similarity_score": 0.89,      "collected_at": "2025-10-24T10:30:00Z"    },    {      "id": "02def...",      "value_text": "Can't complete my order, keeps timing out.",      "field_label": "Issues?",      "sentiment": "negative",      "topics": ["order", "performance"],      "similarity_score": 0.84,      "collected_at": "2025-10-24T09:15:00Z"    }  ],  "query": "checkout problems",  "count": 2}
```

## How Embeddings Are Generated[​](#how-embeddings-are-generated "Direct link to How Embeddings Are Generated")

### Automatic Background Processing[​](#automatic-background-processing "Direct link to Automatic Background Processing")
...
### Context-Aware Embeddings[​](#context-aware-embeddings "Direct link to Context-Aware Embeddings")

Embeddings include question context for better matching:

```
"Question: What did you like most?Response: The fast checkout process"
```
...
### Storage with pgvector[​](#storage-with-pgvector "Direct link to Storage with pgvector")
...
## Search API Reference[​](#search-api-reference "Direct link to Search API Reference")
...
```
# Current ticket: "User can't log in with Google"GET /v1/feedback-records/search?query=google+login+not+working&source_type=support&limit=10
```
...
### Product Discovery[​](#product-discovery "Direct link to Product Discovery")

**Discover emerging themes:**

```
# What are users saying about our mobile app?GET /v1/feedback-records/search?query=mobile+app+feedback+record&limit=100
```
...
```
# What are users comparing us to?GET /v1/feedback-records/search?query=competitor+comparison+features
```
...
```
# Q1 pricing feedbackGET /v1/feedback-records/search?query=pricing+expensive+cost&since=2025-01-01&until=2025-03-31# Q2 pricing feedbackGET /v1/feedback-records/search?query=pricing+expensive+cost&since=2025-04-01&until=2025-06-30
```
...
### Pricing (text-embedding-3-small)[​](#pricing-text-embedding-3-small "Direct link to Pricing (text-embedding-3-small)")
...
```
# Worker pool settingsSERVICE_ENRICHMENT_WORKERS=3                    # Concurrent workers (default: 3)SERVICE_ENRICHMENT_POLL_INTERVAL=1              # Poll interval in seconds (default: 1)# Embedding modelSERVICE_OPENAI_EMBEDDING_MODEL=text-embedding-3-small  # Default
```
...
### Check Embedding Coverage[​](#check-embedding-coverage "Direct link to Check Embedding Coverage")

See how many responses have been embedded:

```
-- Embedding progressSELECT   COUNT(*) FILTER (WHERE embedding IS NOT NULL) as embedded,  COUNT(*) FILTER (WHERE embedding IS NULL) as not_embedded,  COUNT(*) as total,  ROUND(100.0 * COUNT(*) FILTER (WHERE embedding IS NOT NULL) / COUNT(*), 1) as percentageFROM feedback_recordsWHERE field_type = 'text' AND value_text IS NOT NULL;
```

### Monitor Embedding Jobs[​](#monitor-embedding-jobs "Direct link to Monitor Embedding Jobs")

Check job queue status:

```
-- Embedding job statusSELECT   status,  COUNT(*) as countFROM enrichment_jobsWHERE job_type = 'embedding'GROUP BY status;-- Recent embedding activitySELECT   id,  status,  created_at,  processed_at,  EXTRACT(EPOCH FROM (processed_at - created_at)) as processing_secondsFROM enrichment_jobsWHERE job_type = 'embedding'ORDER BY created_at DESCLIMIT 20;
```

### Verify Search Index[​](#verify-search-index "Direct link to Verify Search Index")

Check HNSW index status:

```
-- Index informationSELECT   indexname,  indexdefFROM pg_indexesWHERE tablename = 'feedback_records'  AND indexname LIKE '%embedding%';
```

## Performance[​](#performance "Direct link to Performance")

### Query Speed[​](#query-speed "Direct link to Query Speed")
...
### Accuracy vs Speed[​](#accuracy-vs-speed "Direct link to Accuracy vs Speed")
...
* [AI Enrichment →](/core-concepts/ai-enrichment) - Understand how sentiment/topics are extracted
* [Data Model →](/core-concepts/data-model) - Explore the feedback record schema
* [Webhooks →](/core-concepts/webhooks) - React to new feedback in real-time
* [API Reference →](/api-reference) - Explore all endpoints

[Edit this page](https://github.com/formbricks/hub/tree/main/apps/docs/docs/core-concepts/semantic-search.md)
