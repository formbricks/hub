...
curl -o docker-compose.yml https://raw.githubusercontent.com/formbricks/hub/main/docker-compose.yml
```
...
docker compose exec postgres pg_isready -U formbricks -d hub

# Apply the schema
docker compose exec postgres psql -U formbricks -d hub -f /dev/stdin < sql/001_initial_schema.sql
```

Or if you have `psql` installed locally:

```bash
psql "postgres://formbricks:your_postgres_password@localhost:5432/hub?sslmode=disable" -f sql/001_initial_schema.sql
```
...
<Warning>
  Make sure to initialize the database schema before starting the API. Download and run the schema file from the [GitHub repository](https://github.com/formbricks/hub/blob/main/sql/001_initial_schema.sql):

  ```bash
  psql "postgresql://user:password@host:5432/hub" -f sql/001_initial_schema.sql
  ```
</Warning>
