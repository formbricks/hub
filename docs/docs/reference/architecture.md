...
│   │   ├── feedbackrecords.go   # CRUD handlers
│   │   └── types.go             # API request/response types
│   ├── config/
│   │   └── config.go            # Configuration management
│   ├── ent/
│   │   ├── schema/
│   │   │   └── feedbackrecord.go  # Ent schema definition
...
│   ├── models/
│   │   └── feedbackrecord.go    # Domain models (business logic)
│   └── webhook/
...
2. Huma validates request against OpenAPI spec
   ↓
3. Handler receives CreateFeedbackRecordInput
   ↓
...
7. Dispatch webhook asynchronously
   ↓
8. Return 201 Created with feedback record data
```

### List Feedback Records with Filters

```
1. GET /v1/feedback-records?source_type=survey&limit=50
   ↓
...
CREATE INDEX idx_source_type ON feedback_records(source_type);
CREATE INDEX idx_source_id ON feedback_records(source_id);
CREATE INDEX idx_collected_at ON feedback_records(collected_at);
CREATE INDEX idx_field_type ON feedback_records(field_type);
CREATE INDEX idx_value_number ON feedback_records(value_number);
CREATE INDEX idx_user_identifier ON feedback_records(user_identifier);

-- Composite indexes
CREATE INDEX idx_source_collected ON feedback_records(source_type, source_id, collected_at);
CREATE INDEX idx_field_collected ON feedback_records(field_type, collected_at);
```
...
```go
// internal/api/feedbackrecords_test.go
...
### Adding a New Field to FeedbackRecord
