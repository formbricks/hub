...
These variables are read by the `@formbricks/hub-mcp` package and the Hub source
repository's `make mcp-smoke` target. They are not read by the Hub API or worker
processes.

| Variable                  | Default                      | Description                                                                      |
| ------------------------- | ---------------------------- | -------------------------------------------------------------------------------- |
| `HUB_API_KEY`             | -                            | API key the MCP package sends to Hub; usually the same value as Hub's `API_KEY`. |
...
| `HUB_MCP_PACKAGE`         | `@formbricks/hub-mcp@latest` | Optional package spec used by `make mcp-smoke`.                                  |
