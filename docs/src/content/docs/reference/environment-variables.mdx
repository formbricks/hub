...
These variables are used by MCP client/package flows and the Hub source
repository's `make mcp-smoke` target. They are not read by the Hub API or worker
processes.

| Variable                  | Default | Description                                                                                                                |
| ------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------- |
| `HUB_API_KEY`             | -       | API key used by MCP clients or smoke tests; usually the same value as Hub's `API_KEY`.                                     |
| `FORMBRICKS_HUB_BASE_URL` | -       | Hub API root used by the MCP package, for example `https://hub.example.com`.                                               |
| `HUB_BASE_URL`            | -       | Legacy alias accepted by the Hub repository smoke test; MCP client configs should use `FORMBRICKS_HUB_BASE_URL`.           |
| `HUB_MCP_PACKAGE`         | -       | Package spec used by `make mcp-smoke`, for example `@formbricks/hub-mcp@latest`; required unless `HUB_MCP_COMMAND` is set. |
| `HUB_MCP_COMMAND`         | -       | Local command used by `make mcp-smoke`; takes precedence over `HUB_MCP_PACKAGE`.                                           |
| `HUB_MCP_ARGS`            | -       | Whitespace-separated arguments passed to `HUB_MCP_COMMAND`.                                                                |
