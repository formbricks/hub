...
| `TRANSLATION_RECONCILE_MAX_CONCURRENT` | `2`  | Max concurrent translation jobs on the reconcile lane (see [Reconciliation](#reconciliation)).                                                          |
...
| `SENTIMENT_RECONCILE_MAX_CONCURRENT` | `2`  | Max concurrent sentiment jobs on the reconcile lane (see [Reconciliation](#reconciliation)).        |
...
| `EMOTIONS_RECONCILE_MAX_CONCURRENT` | `2`  | Max concurrent emotion jobs on the reconcile lane (see [Reconciliation](#reconciliation)).       |
...
Enrichment is enqueued by events as records arrive, which is fast but not complete: a record
created while a provider was unconfigured has no job at all, a job can be lost to a crash, and a
transient outage can exhaust a record's retries. The reconciler closes that gap. It periodically
finds every record that is eligible for an enrichment, has not received one, and has not failed
terminally, and enqueues the difference. Events remain the fast path; this is what makes coverage
eventually complete.

It runs on `hub-worker`, on River's elected leader, so a deployment with several workers sweeps
once between them rather than once each. Disabling it does not affect the event path — it only
stops coverage from healing itself.

| Variable                                | Default | Description                                                                                                       |
| --------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------- |
| `ENRICHMENT_RECONCILE_ENABLED`          | `true`  | Master switch for the periodic sweep. **Set it identically on hub-api and hub-worker** — see below.               |
| `ENRICHMENT_RECONCILE_INTERVAL_SECONDS` | `300`   | How often the sweep runs.                                                                                         |
| `ENRICHMENT_RECONCILE_TARGET_DEPTH`     | `1000`  | Per-queue top-up target: each tick adds only enough to bring a reconcile queue to this depth. Clamped to `100000`. |

Reconciled work runs on its own `*_reconcile` queue per enrichment, with its own worker budget
(`*_RECONCILE_MAX_CONCURRENT` above), so draining a large backlog cannot crowd out a record
submitted a moment ago. Topping up *to* a depth rather than enqueueing *at* a rate makes the sweep
self-regulating — a tick can only add what the workers have already drained — so the job table
stays bounded whether the backlog is a thousand records or millions.

`ENRICHMENT_RECONCILE_ENABLED` must carry the same value on **both** `hub-api` and `hub-worker`.
The sweep itself runs on the worker, but the API reads the flag too, to decide whether clearing
terminal failures could ever lead to anything. Disable it on the worker alone — the natural thing
to do for a worker-only feature, since that is where the Helm chart's `worker.extraEnv` applies —
and the API still accepts a retry: it deletes the markers, spends the tenant's cooldown, drops
those records out of the status endpoint's failed counts, and no sweep ever picks them up. Set it
on the API alone and every retry is refused with a 409 while the worker sweeps normally.

Records that failed **terminally** are excluded. A terminal failure means the provider rejected the
text on structural grounds — a content-policy block, a refusal, input past the model's limit — so
retrying spends a provider call that cannot succeed, and a sweep that re-enqueued them would never
terminate. To give those records another chance after the underlying reason has changed (the text
was edited, or a provider relaxed a policy), clear their markers with
`POST /v1/tenants/{tenant_id}/enrichments/retry`. That endpoint is rate-limited by a per-tenant,
per-enrichment cooldown, because clearing the markers is what makes the next sweep spend money on
records already known to fail.

## Taxonomy

[Taxonomy](/core-concepts/taxonomy) clusters embedded feedback into a tree of topics via a
standalone service. It requires embeddings (see [Embeddings](#embeddings)) to be configured.
...
With `OTEL_METRICS_EXPORTER` set, Hub emits the metrics listed in the [Metrics](/reference/metrics) reference.
