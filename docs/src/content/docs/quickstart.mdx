...
<Callout variant="note">
  You only need to run this once when setting up the database for the first
  time.
</Callout>
...
<Callout variant="note">
  <strong>Warning:</strong> Make sure to run migrations before starting the API.
  From the Hub repo with `DATABASE_URL` set: `make init-db` or `goose -dir
  migrations postgres "$DATABASE_URL" up`.
</Callout>
