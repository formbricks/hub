...
Migrations are managed with [goose](https://github.com/pressly/goose) and [River](https://riverqueue.com/). The Hub image includes both migration CLIs and the migration files:

```bash
docker compose run --rm --entrypoint /bin/sh hub -c \
...
<Callout variant="note">
  ...
  The Hub image includes `goose`, `river`, and `/app/migrations`; run the same
  migration command from the Docker Compose setup against your database.
</Callout>
