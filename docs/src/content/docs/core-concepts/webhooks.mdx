...
Start Hub with the [Quick Start Docker Compose setup](/quickstart). It applies
database migrations and River migrations automatically before starting the API
and worker:
...
1. Confirm `hub-migrate` completed successfully: `docker compose ps --all hub-migrate`
