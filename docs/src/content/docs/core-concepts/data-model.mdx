...
Translation is gated by the tenant's `target_language`; sentiment and emotions by the per-tenant [`sentiment_enabled` / `emotions_enabled`](/core-concepts/tenant-settings) switches. See the [API Reference](/api) for the exact `sentiment` and `emotions` value sets.
