...
2. Run Hub, worker, and Postgres locally ([Quick Start](/quickstart)).
...
<Aside type="note">
  Power BI Desktop is only available on Windows. For Mac or Linux, we recommend
  using [Apache Superset](/guides/hub-superset) with Hub - it connects to the
  same data.
</Aside>
...
Note the **database connection details** you use for Hub (host, port, database name, user, password). You will need them for Power BI. If you use the [compose file](/quickstart#option-1-docker-compose-recommended) from the docs, that is typically:
...
3. Enter:
   ...
   - **Database** - The database name from the [Step 1](#step-1-get-hub-running) table (for example, `hub`).
...
<Aside type="note">
  If Postgres is on another machine or in the cloud, use that host and ensure
  the firewall or security groups allow Power BI (or the on-premises data
  gateway) to reach port 5432.
</Aside>

<Aside type="note">
  For scheduled refresh or sharing in Power BI Service when the database is not
  publicly reachable, configure an [on-premises data
  gateway](https://learn.microsoft.com/en-us/power-bi/connect-data/service-gateway-onprem)
  and use it for the PostgreSQL connection.
</Aside>
...
- [Connecting Hub to Databricks](/guides/hub-databricks) - Same data, with Databricks JDBC or Lakehouse Federation
- [Connecting Hub to Airbyte](/guides/hub-airbyte) - Replicate Hub data into Snowflake, BigQuery, Redshift, or Databricks on a schedule
