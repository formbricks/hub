...
4. Click **Set up source**. Airbyte will run a connection check and discover the schema. You will pick `updated_at` as the cursor field per stream in [Step 5](#step-5-create-the-connection).
...
<Aside type="note">
  ...
  Airbyte just needs a user with `SELECT` on `feedback_records`. Use **CDC**
  if you need delete tracking or are working with very large tables (500 GB+);
  the Postgres source docs cover the extra setup.
</Aside>
...
<Aside type="note">
  The grant below gives Airbyte access to **all rows in `feedback_records`,
  across every tenant**. This is the right default for an internal analytics
  warehouse where you want cross-tenant reporting on your own Hub. If you are
  building a per-tenant pipeline (for example, replicating a single customer's
  data into their own warehouse), grant on a tenant-scoped view instead - see
  the next subsection.
</Aside>
...
- [Connecting Hub to Databricks](/guides/hub-databricks/) - Direct JDBC or Lakehouse Federation when you do not need ETL
- [Connecting Hub to Superset](/guides/hub-superset/) - Direct dashboards on Hub's Postgres
- [Connecting Hub to Power BI](/guides/hub-powerbi/) - Direct reports on Hub's Postgres
