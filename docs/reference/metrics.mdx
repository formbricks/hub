...
description: "Prometheus metrics, distributed tracing, and correlation (X-Request-ID, trace_id in logs)."
---
...
- **Prometheus metrics** at `GET /metrics` (HTTP request metrics, event pipeline, webhook delivery).
- **Distributed tracing** (HTTP and optional pipeline spans) when an OTLP or stdout exporter is configured.
- **Structured log correlation**: `trace_id`, `span_id`, and `request_id` are added to log records when tracing is enabled and when the request passes through the Request ID middleware.

When metrics or tracing are disabled (env vars empty or unset), a startup warning is logged and the corresponding providers are not created. The `/metrics` endpoint is not registered when metrics are disabled.
...
|----------|-------------|-----------------|
| `OTEL_METRICS_EXPORTER` | Set to `prometheus` to enable metrics and expose `GET /metrics`. Empty/unset = metrics disabled; a warning is logged at startup. | Metrics enabled |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | Optional. OTLP endpoint (e.g. `https://localhost:4318`) for trace export when `OTEL_TRACES_EXPORTER=otlp`. | Used only when tracing is enabled with OTLP |
...
When tracing is enabled, the default slog handler is wrapped so that each log record that receives a context with an active span gets **`trace_id`** and **`span_id`** (W3C string format) added automatically. The same wrapper adds **`request_id`** when the context contains the request ID (set by the Request ID middleware). Use `slog.InfoContext(ctx, ...)` (and other `*Context` variants) in request and worker paths so that the context carries the span and request ID.
...
Hub exposes **Prometheus** metrics at `GET /metrics`. Enable them and have SigNoz scrape the Hub:
...
2. In your SigNoz deployment, configure the OpenTelemetry Collector to scrape the Hub’s `/metrics` endpoint using the [Prometheus receiver](https://signoz.io/docs/instrumentation/opentelemetry-collector/#prometheus-receiver). Add a scrape config targeting the Hub URL (e.g. `http://hub:8080/metrics` in Docker/Kubernetes, or `http://localhost:8080/metrics` for local).
...
| Metrics | `OTEL_METRICS_EXPORTER=prometheus` | Collector scrapes `http://<hub>:8080/metrics` (Prometheus receiver) |
