...
| `hub_webhook_enqueue_retries_total` | — | Total enqueue retry attempts (InsertMany failed, then slept and retried). Use with `hub_webhook_provider_errors_total{reason="enqueue_failed"}` to see retries vs final failures. |
| `hub_cache_hits_total` | `cache` | Number of lookups that returned a cached value (`webhook_list`, `webhook_get_by_id`). Hit ratio = `rate(hub_cache_hits_total) / (rate(hub_cache_hits_total) + rate(hub_cache_misses_total))` per cache. |
| `hub_cache_misses_total` | `cache` | Number of lookups that missed and triggered a load (`webhook_list`, `webhook_get_by_id`). |
| `hub_request_body_too_large_total` | — | Number of requests rejected because the body exceeded `MAX_REQUEST_BODY_BYTES` (413 Request Entity Too Large). |
...
**Sampling:** By default all traces are sampled. In production with high traffic, set `OTEL_TRACES_SAMPLER=parentbased_traceidratio` and `OTEL_TRACES_SAMPLER_ARG=0.1` (or another ratio) to reduce volume. See [Environment Variables](/reference/environment-variables#otel_traces_sampler).
...
| `hub_webhook_enqueue_retries_total` rate high (optional) | Info | Enqueue is often retrying; may indicate transient DB/River load; consider tuning backoff or capacity. |
...
| Cache hit ratio low for `webhook_list` or `webhook_get_by_id` (optional) | Info | If hit ratio drops, consider increasing `WEBHOOK_CACHE_SIZE` or investigate hot keys. |
