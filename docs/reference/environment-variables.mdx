...
Set to `prometheus` to enable Prometheus metrics and expose `GET /metrics`. Empty or unset = metrics disabled.
...
# Optional: metrics and tracing (e.g. Prometheus + OTLP collector)
# OTEL_METRICS_EXPORTER=prometheus
# OTEL_TRACES_EXPORTER=otlp
