...
func NewWorkerApp(cfg *config.Config, db *pgxpool.Pool) (*WorkerApp, error) {
	...
	if providerName != "" {
		...
		if err := service.ValidateEmbeddingConfig(embeddingCfg); err != nil {
			shutdownWorkerObservability(context.Background(), meterProvider, tracerProvider)
	...
	riverWorkers, queueCfgs := workers.NewRiverWorkersAndQueues(cfg, deps, 0)
...
func shutdownWorkerObservability(ctx context.Context, meter *sdkmetric.MeterProvider, tracer *sdktrace.TracerProvider) {
	if tracer != nil {
