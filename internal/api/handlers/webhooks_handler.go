...
func (h *WebhooksHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		slog.Error("Failed to get webhook",
			"method", r.Method,
			"path", r.URL.Path,
			"id", id,
			"error", err,
...
func (h *WebhooksHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("Failed to list webhooks",
			"method", r.Method,
			"path", r.URL.Path,
			"error", err,
		)
...
func (h *WebhooksHandler) Update(w http.ResponseWriter, r *http.Request) {
	idStr := r.PathValue("id")
	if idStr == "" {
		slog.Warn("Missing webhook ID for update",
			"method", r.Method,
			"path", r.URL.Path,
		)
		response.RespondBadRequest(w, "Webhook ID is required")
	...
	if err != nil {
		if errors.Is(err, apperrors.ErrNotFound) {
			slog.Warn("Webhook not found for update",
				"method", r.Method,
				"path", r.URL.Path,
				"id", id,
				"error", err,
		...
		slog.Error("Failed to update webhook",
			"method", r.Method,
			"path", r.URL.Path,
			"id", id,
			"error", err,
...
func (h *WebhooksHandler) Delete(w http.ResponseWriter, r *http.Request) {
	idStr := r.PathValue("id")
	if idStr == "" {
		slog.Warn("Missing webhook ID for delete",
			"method", r.Method,
			"path", r.URL.Path,
		)
		response.RespondBadRequest(w, "Webhook ID is required")
	...
	if err := h.service.DeleteWebhook(r.Context(), id); err != nil {
		if errors.Is(err, apperrors.ErrNotFound) {
			slog.Warn("Webhook not found for delete",
				"method", r.Method,
				"path", r.URL.Path,
				"id", id,
				"error", err,
