...
func GenerateOpenAPISpec(cfg *config.Config, client *ent.Client, dispatcher *webhook.Dispatcher, enrichmentQueue queue.Queue, logger *slog.Logger) (*huma.OpenAPI, error) {
...
// ExportOpenAPISpecJSON exports the OpenAPI spec to a writer in JSON format
func ExportOpenAPISpecJSON(cfg *config.Config, client *ent.Client, dispatcher *webhook.Dispatcher, enrichmentQueue queue.Queue, logger *slog.Logger, w io.Writer) error {
...
// ExportOpenAPISpecYAML exports the OpenAPI spec to a writer in YAML format
func ExportOpenAPISpecYAML(cfg *config.Config, client *ent.Client, dispatcher *webhook.Dispatcher, enrichmentQueue queue.Queue, logger *slog.Logger, w io.Writer) error {
	spec, err := GenerateOpenAPISpec(cfg, client, dispatcher, enrichmentQueue, logger)
	if err != nil {
		return fmt.Errorf("failed to generate OpenAPI spec: %w", err)
	}
...
// ServeOpenAPISpec is a handler that serves the OpenAPI spec in JSON format
func ServeOpenAPISpec(cfg *config.Config, client *ent.Client, dispatcher *webhook.Dispatcher, enrichmentQueue queue.Queue, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := ExportOpenAPISpecJSON(cfg, client, dispatcher, enrichmentQueue, logger, w); err != nil {
