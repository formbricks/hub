...
func (w *TaxonomyScheduler) processJob(ctx context.Context, job models.ClusteringJob) {
	...
	if err != nil {
		...
		errMsg := err.Error()
		if updateErr := w.repo.UpdateAfterRun(ctx, job.ID, &models.UpdateClusteringJobRequest{
			Status:    models.JobStatusFailed,
			LastError: &errMsg,
		}); updateErr != nil {
			logger.Error("failed to update job status after trigger failure", "error", updateErr)
		}
		return
	}
	...
	for {
		...
		case <-timeout:
			...
			if updateErr := w.repo.UpdateAfterRun(ctx, job.ID, &models.UpdateClusteringJobRequest{
				...
				logger.Error("failed to update job status after timeout", "error", updateErr)
			}
			return
		case <-ticker.C:
